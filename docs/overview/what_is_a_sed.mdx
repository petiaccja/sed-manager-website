# What's a self-encrypting drive?
...
Conveniently enough, the block organization of storage devices is very similar to the block ciphers we've introduced in the previous chapter. While the block sizes for the storage device and the symmetric encryption algorithm may be different, let's just assume for simplicity that they are the same.
